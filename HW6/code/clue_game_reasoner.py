...
class ClueGameReasoner:
    ...
    def __init__(self, player_order, card_nums = None):
        ...
        for card_cat in (SUSPECTS, WEAPONS, ROOMS):
            clause = ""
            for c in card_cat:
    ...
    def suggest(self, suggester, c1, c2, c3, refuter, cardshown = None):
        ...
        # TO BE IMPLEMENTED AS AN EXERCISE
        if refuter is None:
            # Nobody refutes. So, nobody but the suggester has any of those three cards.
            for p in POSSIBLE_PLAYERS:
                if p != suggester:
                    for c in (c1, c2, c3):
                        self.KB.add_clause("~" + c + "_" + p)
        else:
            # at least one of the suggested cards is not in the case file.
            # Because refuter must have at least one of them to refute
            clause = ""
            for c in [c1, c2, c3]:
                clause += "~" + c + "_" + CASE_FILE + " "
            self.KB.add_clause(clause)

            # Everybody between the suggester and refuter does not have any of the suggested cards
            sug_idx = POSSIBLE_PLAYERS.index(suggester)
            ref_idx = POSSIBLE_PLAYERS.index(refuter)
            itr = (sug_idx+1)%len(POSSIBLE_PLAYERS)
            while itr != ref_idx:
                p = POSSIBLE_PLAYERS[itr]
                for c in [c1, c2, c3]:
                    clause = "~" + c + "_" + p
                    self.KB.add_clause(clause)
                itr = (itr+1)%len(POSSIBLE_PLAYERS)

            if cardshown is not None:
                # Refuter has the card because she shows us
                clause = cardshown + "_" + refuter
                self.KB.add_clause(clause)
            else:
                # We do not know. But the refuter has at least one of the suggested cards
                clause = ""
                for c in [c1, c2, c3]:
                    clause += c + "_" + refuter + " "
                self.KB.add_clause(clause)
    ...
    def accuse(self, accuser, c1, c2, c3, iscorrect):
        ...
        # TO BE IMPLEMENTED AS AN EXERCISE
        if iscorrect:
            # Add the info that the three cards are in the case file
            for c in [c1, c2, c3]:
                clause = c + "_" + CASE_FILE
                self.KB.add_clause(clause)
        else:
            # at least one the cards is not in the case file
            clause = ""
            for c in [c1, c2, c3]:
                clause += "~" + c + "_" + CASE_FILE + " "
            self.KB.add_clause(clause)
        # Accuser should not have the three cards. Otherwise, she would not have made accusation
        for c in [c1, c2, c3]:
            clause = "~" + c1 + "_" + accuser
            self.KB.add_clause(clause)
