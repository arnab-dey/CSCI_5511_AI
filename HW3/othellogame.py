...
def get_legal_actions_state(state):
    '''Return a list of possible actions given the current state
    '''
    legal_actions = []
    legal_states = []
    for i in range(SIZE):
        for j in range(SIZE):
            result_state = result(state, (i,j))
            if result_state != None:
                legal_actions.append((i,j))
                legal_states.append(result_state)
    if len(legal_actions) == 0:
        legal_actions.append(SKIP)
        legal_states.append(OthelloState(state.other, state.current, copy.deepcopy(state.board_array), state.num_skips + 1))
    return legal_actions, legal_states
