...
def utility(state, player, is_mobility_reqd=False):
    ...
    for i in range(og.SIZE):
        for j in range(og.SIZE):
            if state.board_array[j][i] == player_color:
                n_player_coins += 1
                player_stability += stability_matrix[j][i]
                if (j,i) in corners:
                    n_player_corner += 1
    ...
    coin_parity = 100*(n_player_coins-n_opponent_coins)/(n_player_coins+n_opponent_coins)
    # Calculate corner utility
    corner_utility = 0
    if (n_player_corner + n_opponent_corner != 0):
        corner_utility = 100 * (n_player_corner - n_opponent_corner) / (n_player_corner + n_opponent_corner)
    # Calculate mobility utility
    mobility_utility = 0
    if (n_player_mobility+n_opponent_mobility != 0):
        mobility_utility = 100*(n_player_mobility-n_opponent_mobility)/(n_player_mobility+n_opponent_mobility)
    ...
    total_utility = (3*coin_parity+1*mobility_utility+2*corner_utility)/600+player_stability+terminal_utility
    ...
    def make_move(self, state):
        ...
        for i, (a,s) in enumerate(zip(legal_actions,legal_states)):
            ...
            if v > best_score:
                ...
                best_action = a
        return best_action
        # return max(legal_actions, key=lambda a,s: self.min_value(s, 1))
...
# CODE STARTS HERE
########################################################
p2 = AlphabetaPlayer(og.WHITE, 4)
p1 = AlphabetaPlayer(og.BLACK, 4)
og.play_game(p1, p2)
